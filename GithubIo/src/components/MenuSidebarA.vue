<template>
  ...
  <div class="floating-toggle-btn"
       :class="{ 'floating-btn-hidden': !isMenuVisible }"
       @click="toggleMenu">
    <i class="toggle-icon" :class="isMenuVisible ? 'icon-hide' : 'icon-show'"></i>
    <span class="toggle-tooltip">{{ isMenuVisible ? '隐藏菜单' : '显示菜单' }}</span>
  </div>

  <lay-side class="layui-menu-ref-1" :class="{ 'menu-collapsed': !isMenuVisible }">
...
const menuWidth = computed(() => isMenuVisible.value ? "200px" : "0px");
...
defineExpose({
  toggleMenu,
  showMenu: () => isMenuVisible.value = true,
  hideMenu: () => isMenuVisible.value = false,
  isMenuVisible
});
</script>
